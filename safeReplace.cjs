...
if (!code.includes('function getThemeStyles')) {
  code = code.replace('function Hero({ v }: { v: Venture }) {', themeFunction + '\nfunction Hero({ v }: { v: Venture }) {');
}

// Global replacement - add theme definition
code = code.replaceAll("const isTacto = v.slug === 'project-tacto'", "const theme = getThemeStyles(v.slug)\n  const isTacto = v.slug === 'project-tacto'");

// Hero Component Replacements
code = code.replaceAll("opacity: isTacto ? 0.18 : 0.22", "opacity: isTacto ? 0.18 : v.slug === 'carbonsynq' ? 0.15 : 0.22");
...
code = code.replaceAll("isTacto\n                  ? 'bg-violet-500/15 border-violet-400/25 text-violet-300'\n                  : 'bg-blue-500/15 border-blue-400/25 text-blue-300'", "isTacto ? 'bg-violet-500/15 border-violet-400/25 text-violet-300' : v.slug === 'carbonsynq' ? 'bg-teal-500/15 border-teal-400/25 text-teal-300' : 'bg-blue-500/15 border-blue-400/25 text-blue-300'");

code = code.replaceAll("absolute inset-0 rounded-[2.5rem] blur-[60px] scale-110 pointer-events-none ${isTacto ? 'bg-violet-500/20' : 'bg-blue-500/15'}", "absolute inset-0 rounded-[2.5rem] blur-[60px] scale-110 pointer-events-none ${isTacto ? 'bg-violet-500/20' : v.slug === 'carbonsynq' ? 'bg-teal-500/20' : 'bg-blue-500/15'}");

code = code.replaceAll("isTacto\n                    ? 'bg-gradient-to-br from-violet-500/20 to-indigo-500/10 border-violet-400/20'\n                    : 'bg-gradient-to-br from-white/20 to-white/5 border-white/20'", "isTacto ? 'bg-gradient-to-br from-violet-500/20 to-indigo-500/10 border-violet-400/20' : v.slug === 'carbonsynq' ? 'bg-gradient-to-br from-teal-500/20 to-cyan-500/10 border-teal-400/20' : 'bg-gradient-to-br from-white/20 to-white/5 border-white/20'");

// MetadataBar
code = code.replaceAll("isTacto\n                  ? 'text-violet-600 bg-violet-50 border-violet-100/60'\n                  : 'text-blue-600 bg-blue-50 border-blue-100/60'", "`${theme.text} ${theme.bg} ${theme.border}`");
...
code = code.replaceAll("isTacto ? \"hover:shadow-violet-100/60\" : \"hover:shadow-blue-100/60\"", "theme.shadow");

code = code.replaceAll("accent={isTacto}", "accent={v.slug === 'project-tacto' || v.slug === 'carbonsynq'}");
...
code = code.replaceAll("isTacto\n                          ? 'bg-violet-500/20 border-violet-400/30'\n                          : 'bg-blue-500/20 border-blue-400/30'", "`${theme.badgeBg} ${theme.badgeBorder}`");
...
code = code.replaceAll("isTacto ? 'bg-gradient-to-br from-violet-500 to-indigo-600' : 'bg-gradient-to-br from-slate-600 to-slate-800'", "`bg-gradient-to-br ${theme.avatar}`");
code = code.replaceAll("isTacto ? 'text-violet-600' : 'text-blue-600'", "theme.text");
code = code.replaceAll("isTacto\n                ? 'Student innovators from Galgotias University, India — building technology that breaks down barriers in STEM education.'\n                : 'Supported by advisors and domain experts across AI, enterprise technology and international markets.'", "v.slug === 'project-tacto' ? 'Student innovators from Galgotias University, India — building technology that breaks down barriers in STEM education.' : v.slug === 'carbonsynq' ? 'Driving climate tech innovation and AI sustainability analytics.' : 'Supported by advisors and domain experts across AI, enterprise technology and international markets.'");

// Roadmap
code = code.replaceAll("{isTacto && (", "{(v.slug === 'project-tacto' || v.slug === 'carbonsynq') && (");
code = code.replaceAll("isTacto\n                              ? i === 0 ? 'bg-violet-600' : i === 1 ? 'bg-indigo-600' : i === 2 ? 'bg-blue-600' : 'bg-teal-600'\n                              : 'bg-slate-900'", "v.slug === 'project-tacto' ? (i === 0 ? 'bg-violet-600' : i === 1 ? 'bg-indigo-600' : i === 2 ? 'bg-blue-600' : 'bg-teal-600') : v.slug === 'carbonsynq' ? (i === 0 ? 'bg-emerald-600' : i === 1 ? 'bg-teal-600' : i === 2 ? 'bg-cyan-600' : 'bg-teal-700') : 'bg-slate-900'");
code = code.replaceAll("isTacto && i === 0 ? 'bg-violet-400' : 'bg-slate-300'", "(v.slug === 'project-tacto' || v.slug === 'carbonsynq') && i === 0 ? (v.slug === 'project-tacto' ? 'bg-violet-400' : 'bg-teal-400') : 'bg-slate-300'");

// GelsRole
code = code.replaceAll("isTacto ? 'bg-[#18082E]' : 'bg-slate-900'", "v.slug === 'project-tacto' ? 'bg-[#18082E]' : v.slug === 'carbonsynq' ? 'bg-[#042A27]' : 'bg-slate-900'");
code = code.replaceAll("{isTacto ? (", "{(v.slug === 'project-tacto' || v.slug === 'carbonsynq') ? (");
code = code.replaceAll('{isTacto && <div className="absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r from-violet-500 via-indigo-400 to-teal-400" />}', '{(v.slug === \'project-tacto\' || v.slug === \'carbonsynq\') && <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${theme.gradient}`} />}');
...
code = code.replaceAll("isTacto\n                ? 'text-violet-100 bg-violet-500/20 border-violet-500/30'\n                : 'text-amber-100 bg-amber-500/20 border-amber-500/30'", "v.slug === 'project-tacto' ? 'text-violet-100 bg-violet-500/20 border-violet-500/30' : v.slug === 'carbonsynq' ? 'text-teal-100 bg-teal-500/20 border-teal-500/30' : 'text-amber-100 bg-amber-500/20 border-amber-500/30'");
code = code.replaceAll("isTacto ? 'bg-violet-50 border-violet-100 text-violet-500' : 'bg-amber-50 border-amber-100 text-amber-500'", "v.slug === 'project-tacto' ? 'bg-violet-50 border-violet-100 text-violet-500' : v.slug === 'carbonsynq' ? 'bg-teal-50 border-teal-100 text-teal-600' : 'bg-amber-50 border-amber-100 text-amber-500'");
code = code.replaceAll("isTacto ? 'bg-violet-50 border-violet-100' : 'bg-amber-50 border-amber-100'", "v.slug === 'project-tacto' ? 'bg-violet-50 border-violet-100' : v.slug === 'carbonsynq' ? 'bg-teal-50 border-teal-100' : 'bg-amber-50 border-amber-100'");
code = code.replaceAll("isTacto ? 'text-violet-500' : 'text-amber-500'", "v.slug === 'project-tacto' ? 'text-violet-500' : v.slug === 'carbonsynq' ? 'text-teal-600' : 'text-amber-500'");
code = code.replaceAll("isTacto ? 'bg-violet-50 border-violet-100 text-violet-500' : 'bg-blue-50 border-blue-100 text-blue-500'", "v.slug === 'project-tacto' ? 'bg-violet-50 border-violet-100 text-violet-500' : v.slug === 'carbonsynq' ? 'bg-teal-50 border-teal-100 text-teal-600' : 'bg-blue-50 border-blue-100 text-blue-500'");
code = code.replaceAll("isTacto ? 'bg-violet-50/80 border-violet-100 text-violet-600' : 'bg-slate-50 border-slate-100 text-slate-600'", "v.slug === 'project-tacto' ? 'bg-violet-50/80 border-violet-100 text-violet-600' : v.slug === 'carbonsynq' ? 'bg-teal-50/80 border-teal-100 text-teal-600' : 'bg-slate-50 border-slate-100 text-slate-600'");
code = code.replaceAll("isTacto\n                ? 'bg-violet-600 hover:bg-violet-700 text-white shadow-violet-200'\n                : 'bg-slate-900 hover:bg-slate-800 text-white shadow-slate-200'", "v.slug === 'project-tacto' ? 'bg-violet-600 hover:bg-violet-700 text-white shadow-violet-200' : v.slug === 'carbonsynq' ? 'bg-teal-600 hover:bg-teal-700 text-white shadow-teal-200' : 'bg-slate-900 hover:bg-slate-800 text-white shadow-slate-200'");
