        ...
        <div className="mx-auto max-w-7xl px-6 flex items-center gap-8 min-w-max">
          {tabs.map((tab) => (
            <button
              ...
              className={`py-4 text-[11px] font-bold uppercase tracking-wider transition-colors relative ${activeTab === tab ? 'text-forest-deep' : 'text-forest/50 hover:text-forest-deep'}`}
            >
              {tab}
              {activeTab === tab && (
  ...
  return (
    <section className="bg-forest-deep text-white pt-10 pb-16 relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        <div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-white/50 mb-8">
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-8">
            <span className="inline-block px-3 py-1 bg-white/10 border border-white/20 rounded text-[10px] font-bold uppercase tracking-widest text-gold mb-6">
              {v.track}
            </span>
            ...
            <p className="text-[13px] text-white/80 leading-relaxed max-w-2xl mb-8">
              {v.intro}
            </p>
          </div>
          ...
          <div className="lg:col-span-4 relative flex flex-col items-center">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-2xl text-forest-deep w-full max-w-sm">
              <h3 className="text-[13px] font-bold mb-2">Interested in this venture?</h3>
              <p className="text-[10px] text-forest/70 mb-6">We facilitate introductions to founders and support meaningful collaborations.</p>
              
              <button className="w-full flex items-center justify-center gap-2 bg-forest-deep text-white rounded-lg py-2.5 text-[11px] font-bold mb-3 hover:bg-forest transition-colors">
                Request Introduction <ArrowUpRight className="h-3 w-3" />
              </button>
              <button className="w-full flex items-center justify-center gap-2 bg-white text-forest-deep border border-forest/20 rounded-lg py-2.5 text-[11px] font-bold hover:bg-forest/5 transition-colors">
                Request Venture Brief <Download className="h-3 w-3" />
              </button>
              
              <div className="flex items-start gap-2 mt-4 text-[9px] text-forest/60">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 divide-x divide-forest/10">
          {items.map((item, i) => (
            <div key={i} className={`flex items-center gap-4 ${i !== 0 ? 'pl-4 md:pl-8' : ''}`}>
              ...
              <div>
                <p className="text-[9px] font-bold uppercase tracking-widest text-forest/50">{item.l}</p>
                <p className="text-[11px] font-semibold text-forest-deep mt-0.5">{item.v}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {problemCards.map((c, i) => (
              <div key={i} className="bg-white rounded-2xl p-6 border border-forest/10 shadow-sm flex flex-col items-center text-center">
                ...
                <h4 className="text-[13px] font-bold text-forest-deep mb-3">{c.title}</h4>
                <p className="text-[10px] text-forest/70 leading-relaxed flex-1 mb-4">{c.desc}</p>
                <button className="text-[10px] font-bold text-forest hover:text-forest-deep inline-flex items-center gap-1 transition-colors mt-auto">
              ...
              <div className="space-y-4 mb-8">
                {v.progress.map((p, i) => (
                  <div key={i} className="flex items-start gap-3">
                    ...
                    <span className="text-[11px] text-forest/80 leading-relaxed">{p}</span>
                  </div>
                ))}
              </div>
              
              <div className="bg-cream rounded-xl p-6 mt-auto">
                <h4 className="text-[11px] font-bold text-forest-deep mb-4">
                ...
                <div className="space-y-3">
                  {v.metrics.map((m, i) => (
                    <div key={i} className="flex items-center justify-between border-b border-forest/10 pb-2 last:border-0 last:pb-0">
                      <span className="text-[10px] text-forest/70 font-medium">{m.l}</span>
                      <div className="text-right">
                        <span className="text-[13px] font-bold text-forest-deep">{m.v}</span>
                        {m.subtitle && <p className="text-[9px] text-forest/50 leading-none">{m.subtitle}</p>}
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="space-y-6">
                {v.team.map((member, i) => (
                  <div key={i} className="flex items-start gap-4 pb-6 border-b border-forest/5 last:border-0 last:pb-0">
                    ...
                    <div>
                      <h4 className="text-[12px] font-bold text-forest-deep">{member.name}</h4>
                      <p className="text-[10px] font-semibold text-gold mb-2">{member.role}</p>
                      <p className="text-[10px] text-forest/70 leading-relaxed">{member.bio}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-6 pt-6 border-t border-forest/5">
                <p className="text-[10px] text-forest/70 italic mb-4">Advisors and domain experts from security, AI and enterprise technology.</p>
                <button className="text-[11px] font-bold text-forest hover:text-forest-deep inline-flex items-center gap-1 transition-colors">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 relative">
              ...
              {v.roadmap.map((stage, i) => (
                <div key={i} className="flex flex-col items-center">
                  ...
                  <h4 className="text-[10px] font-bold text-forest-deep mb-4">{stage.period}</h4>
                  <ul className="text-left w-full space-y-2">
                    {stage.items.map((item, j) => (
                      <li key={j} className="flex items-start gap-2 text-[10px] text-forest/70 leading-relaxed">
            ...
            <div className="space-y-4">
              {v.highlights.map((h, i) => (
                <div key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-[11px] text-forest/80 leading-relaxed">{h}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-6">
              {v.seeking.map((s, i) => (
                <div key={i} className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="text-[11px] font-bold text-forest-deep mb-1">{s.t}</h4>
                    <p className="text-[10px] text-forest/70 leading-relaxed">{s.d}</p>
                  </div>
                </div>
              ))}
            </div>
            <button className="w-full mt-6 bg-white border border-forest/20 text-forest-deep rounded-lg py-3 text-[11px] font-bold hover:bg-forest/5 transition-colors inline-flex items-center justify-center gap-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            ...
            <div key={i} className="flex items-start gap-3">
              ...
              <span className="text-[10px] font-medium leading-relaxed">{item}</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <section className="bg-white border-t border-forest/10 py-12">
      <div className="mx-auto max-w-7xl px-6 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex gap-4 shrink-0">
          <button className="bg-gold hover:bg-gold/90 text-forest-deep px-6 py-3 rounded-lg text-[11px] font-bold transition-colors inline-flex items-center gap-2">
            Request Introduction <ArrowUpRight className="h-3 w-3" />
          </button>
          <button className="bg-white border border-forest/20 text-forest-deep px-6 py-3 rounded-lg text-[11px] font-bold hover:bg-forest/5 transition-colors inline-flex items-center gap-2">
            Request Venture Brief <Download className="h-3 w-3" />
          </button>
        </div>
      </div>
