...
import {
  ...
  Users, Target, Rocket, Presentation, Brain, Trophy
} from 'lucide-react'
  ...
  return (
    <section className="relative bg-[#F4EFE6] overflow-hidden pt-20 pb-36 md:pt-28 md:pb-44">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <Link
          to="/students"
          className="inline-flex items-center gap-2 text-[13px] font-bold uppercase tracking-[0.18em] text-gold mb-10 hover:text-gold/70 transition-colors"
        >
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div>
            {/* badge */}
            <span className="inline-flex items-center gap-2 bg-gold/12 text-gold text-[11px] font-bold uppercase tracking-[0.22em] px-4 py-2 rounded-full border border-gold/25 mb-7">
            ...
            <div className="flex gap-4 flex-wrap">
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 rounded-full bg-gold px-8 py-4 text-[13px] font-bold uppercase tracking-[0.18em] text-forest-deep hover:bg-amber-400 transition-all duration-300 shadow-xl shadow-gold/25"
              >
                Apply Now <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link
                to="/students"
                className="inline-flex items-center gap-2 rounded-full border border-forest/20 bg-white px-7 py-4 text-[13px] font-bold uppercase tracking-[0.18em] text-forest hover:border-gold/40 transition-all duration-300"
              >
                View All Tracks
              </Link>
            </div>
...
function WhatYoullDo() {
  const pillars = [
    { icon: <Lightbulb className="h-6 w-6" />, title: 'Ideation', desc: 'Learn structured frameworks to generate, evaluate and develop business ideas.' },
    { icon: <TrendingUp className="h-6 w-6" />, title: 'Strategy', desc: 'Understand market dynamics, business models and competitive positioning.' },
    { icon: <Presentation className="h-6 w-6" />, title: 'Pitching', desc: 'Craft and deliver compelling pitches to mentors and industry leaders.' },
    { icon: <Brain className="h-6 w-6" />, title: 'Innovation', desc: 'Use design thinking and AI tools to solve real-world problems.' },
  ]
  return (
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-14">
          <div className="flex items-center justify-center gap-3 mb-5">
            ...
            <span className="text-[12px] font-bold uppercase tracking-[0.3em] text-gold">What You'll Do</span>
            <div className="h-px w-8 bg-gold" />
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {pillars.map((p, i) => (
            <div key={i} className="group rounded-[1.5rem] border border-forest/8 bg-white p-7 hover:border-gold/35 hover:shadow-[0_16px_48px_rgba(26,53,35,0.10)] hover:-translate-y-1 transition-all duration-400 shadow-sm">
...
function Modules() {
  const modules = [
    { num: '01', title: 'Opportunity Discovery', items: ['Spotting market gaps', 'Customer empathy', 'Problem-first thinking'] },
    { num: '02', title: 'Business Model Basics', items: ['Revenue models', 'Value proposition', 'Cost structure'] },
    { num: '03', title: 'Pitching & Storytelling', items: ['Narrative structure', 'Slide design', 'Voice & presence'] },
    { num: '04', title: 'AI for Innovation', items: ['AI tools overview', 'Prototyping with AI', 'Future of work'] },
    { num: '05', title: 'Design Thinking', items: ['Ideate → Prototype', 'User testing', 'Rapid iteration'] },
    { num: '06', title: 'Venture Challenge', items: ['Team formation', 'Pitch to mentors', 'Feedback & reflection'] },
  ]
  return (
    <section className="py-24 bg-[#F4EFE6]">
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-14">
          <div className="flex items-center justify-center gap-3 mb-5">
            <div className="h-px w-8 bg-gold" />
            <span className="text-[12px] font-bold uppercase tracking-[0.3em] text-gold">Programme Modules</span>
            <div className="h-px w-8 bg-gold" />
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {modules.map((m, i) => (
            <div key={i} className="rounded-[1.5rem] bg-white border border-forest/8 p-7 hover:border-gold/30 hover:shadow-[0_12px_36px_rgba(26,53,35,0.09)] transition-all duration-300 shadow-sm group">
              <span className="text-[13px] font-black text-gold/50 tracking-[0.2em] group-hover:text-gold transition-colors duration-300">{m.num}</span>
        ...
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div>
            <div className="flex items-center gap-3 mb-5">
              <div className="h-px w-8 bg-gold" />
              <span className="text-[12px] font-bold uppercase tracking-[0.3em] text-gold">What You'll Leave With</span>
            </div>
            ...
            {[
              { icon: <Trophy className="h-5 w-5" />, text: 'Venture Challenge certificate from Cambridge' },
              { icon: <Users className="h-5 w-5" />, text: 'Network of like-minded young innovators' },
              { icon: <Rocket className="h-5 w-5" />, text: 'A validated business idea ready to grow' },
              { icon: <Target className="h-5 w-5" />, text: 'Clarity on your entrepreneurial path' },
            ].map((o, i) => (
  ...
  return (
    <section className="bg-[#F4EFE6] py-24 text-center">
      <div className="mx-auto max-w-2xl px-6">
        <div className="flex items-center justify-center gap-3 mb-5">
          <div className="h-px w-8 bg-gold" />
          <span className="text-[12px] font-bold uppercase tracking-[0.3em] text-gold">Join the Track</span>
          <div className="h-px w-8 bg-gold" />
        </div>
        ...
        <Link
          to="/contact"
          className="inline-flex items-center gap-2 rounded-full bg-gold px-10 py-4 text-[13px] font-bold uppercase tracking-[0.18em] text-forest-deep hover:bg-amber-400 transition-all duration-300 shadow-xl shadow-gold/25"
        >
