...
// Check meta tags
const authorMeta = html.match(/<meta\s+property=["']article:author["']\s+content=["']([^"']*)["']/i);
const pubDateMeta = html.match(/<meta\s+property=["']article:published_time["']\s+content=["']([^"']*)["']/i);
...
const postContentMatches = [...html.matchAll(/class=["'][^"']*post-content[^"']*["'][\s\S]*?>([\s\S]*?)<\/div>/gi)];
