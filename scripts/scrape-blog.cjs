...
function fetchUrl(url) {
  return new Promise((resolve, reject) => {
    https
      .get(
        url,
        { headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) BlogMigrator/1.0" } },
        (res) => {
...
function extractMeta(html, tag, attr = "content") {
  ...
  const reg3 = new RegExp(
    `<meta\\s+[^>]*property=["']${tag}["'][^>]*${attr}=["']([^"']*)["']`,
    "i",
  );
  const reg4 = new RegExp(
    `<meta\\s+[^>]*${attr}=["']([^"']*)["'][^>]*property=["']${tag}["']`,
    "i",
  );
...
function mapToWixCategory(rawSlug) {
  if (
    rawSlug.includes("greenschool") ||
    rawSlug.includes("green-school") ||
    rawSlug.includes("swadhaa")
  ) {
  ...
  if (
    rawSlug.includes("cambridge") ||
    rawSlug.includes("oxford") ||
    rawSlug.includes("leadership") ||
    rawSlug.includes("impact-tour")
  ) {
...
function parseBlocksFromHtml(htmlContent) {
  ...
  const elementReg =
    /<(h[1-6]|p|blockquote|li|figure|img)[^>]*>([\s\S]*?)<\/\1>|<img\s+[^>]*\/?>/gi;
  let match;
  ...
  while ((match = elementReg.exec(clean)) !== null) {
    ...
    if (tag.startsWith("h")) {
      ...
      const srcMatch =
        match[0].match(/src=["']([^"']*)["']/i) || match[0].match(/data-src=["']([^"']*)["']/i);
...
async function main() {
  ...
  const urlMatches = [
    ...sitemapRes.data.matchAll(/<loc>(https:\/\/www\.globaledulab\.com\/post\/[^<]+)<\/loc>/g),
  ];
  ...
  for (let i = 0; i < postUrls.length; i++) {
    ...
    try {
      ...
      const sitemapLastModMatch = sitemapRes.data.match(
      ...
      for (let j = 0; j < inlineImgMatches.length; j++) {
        ...
        const srcMatch =
          tag.match(/src=["']([^"']*)["']/i) || tag.match(/data-src=["']([^"']*)["']/i);
