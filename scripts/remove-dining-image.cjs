...
posts.forEach((post) => {
  if (post.rawHtml && post.rawHtml.includes(targetFilename)) {
    ...
    const reg = new RegExp(
      `<figure[^>]*>[\\s\\S]*?${targetFilename}[\\s\\S]*?<\\/figure>|<div[^>]*class=["'][^"']*gallery[^"']*["'][^>]*>[\\s\\S]*?${targetFilename}[\\s\\S]*?<\\/div>|<img[^>]*${targetFilename}[^>]*\\/?>`,
      "gi",
    );
    ...
    if (post.images) {
      post.images = post.images.filter(
        (img) => !img.original.includes(targetFilename) && !img.local.includes(targetFilename),
      );
    }
