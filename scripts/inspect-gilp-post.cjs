...
const url = 'https://www.globaledulab.com/post/inside-the-global-india-leadership-programme-redefining-leadership-in-the-ai-era';
...
https.get(url, res => {
  ...
  res.on('end', () => {
    ...
    const timeMatch = d.match(/data-hook=["']time-to-read["'][^>]*>([\s\S]*?)<\/(?:span|div|p|a)>/i);
    console.log('Time to read:', timeMatch ? timeMatch[1].replace(/<[^>]+>/g, '').trim() : 'Not found');

    console.log('--- TIME AGO / PUBLISHED DATE TEXT ---');
    const timeAgoMatch = d.match(/data-hook=["']time-ago["'][^>]*>([\s\S]*?)<\/(?:span|div|p|a)>/i);
    console.log('Time ago:', timeAgoMatch ? timeAgoMatch[1].replace(/<[^>]+>/g, '').trim() : 'Not found');
  });
});
