...
// Basic setups
code = code.replace(
  /const isTacto = v\.slug === 'project-tacto'/g,
  "const theme = getThemeStyles(v.slug)",
);

// Direct regex replacements
code = code.replace(
  /isTacto\s*\?\s*'text-violet-600 bg-violet-50 border-violet-100\/60'\s*:\s*'text-blue-600 bg-blue-50 border-blue-100\/60'/g,
...
code = code.replace(
  /isTacto \? "hover:shadow-violet-100\/60" : "hover:shadow-blue-100\/60"/g,
  "theme.shadow",
);
...
code = code.replace(
  /isTacto\s*\?\s*'bg-violet-500\/20 border-violet-400\/30'\s*:\s*'bg-blue-500\/20 border-blue-400\/30'/g,
...
code = code.replace(
  /isTacto \? 'bg-gradient-to-br from-violet-500 to-indigo-600' : 'bg-gradient-to-br from-slate-600 to-slate-800'/g,
...
let newText2 =
  "v.slug === 'project-tacto' ? ' Our mission is to help TACTO reach every visually impaired learner across the globe.' : v.slug === 'carbonsynq' ? ' Our mission is to scale CarbonSynq\\'s platform across global markets and establish it as the standard for AI-driven carbon accounting.' : ' Our objective is to help the venture scale globally through the UK and beyond.'";
...
// Bottom CTA and more
code = code.replace(
  /isTacto\s*\?\s*'bg-violet-500\/15 border-violet-500\/20 text-violet-400'\s*:\s*'bg-blue-500\/15 border-blue-500\/20 text-blue-400'/g,
  "`${theme.iconBg} ${theme.iconBorder} ${theme.textAccent}`",
);
code = code.replace(
  /isTacto \? 'bg-violet-50 border-violet-100 text-violet-600' : 'bg-slate-50 border-slate-200 text-slate-700'/g,
...
code = code.replace(
  /isTacto\s*\?\s*'bg-violet-600 hover:bg-violet-500'\s*:\s*'bg-slate-900 hover:bg-slate-800'/g,
