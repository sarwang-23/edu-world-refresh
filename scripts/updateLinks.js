...
// Mapping from file name to human‑readable source label
const titleMap = {
  ...
  "about.tsx": "About Us",
  "index.tsx": "Home Page",
  // ventures will be handled separately
};
...
function updateFile(filePath, title) {
  ...
  content = content.replace(regex, (match, p1, p2, p3) => {
    // If a search prop already exists, leave unchanged
...
fs.readdirSync(routesDir).forEach((file) => {
  ...
  if (file === "ventures.$slug.tsx") {
    // special handling – replace to="/contact" with dynamic v.name
    let content = fs.readFileSync(fullPath, "utf8");
    const regex = /<Link([^>]*?)to="(\/contact)"([^>]*?)>/g;
    content = content.replace(regex, (match, p1, p2, p3) => {
      if (match.includes("search=")) return match;
      return `<Link${p1}to="${p2}" search={{ source: v.name }}${p3}>`;
    });
    // apply‑now links still need a static title – use page title if available
  ...
  if (!title) return; // skip files without a defined title
